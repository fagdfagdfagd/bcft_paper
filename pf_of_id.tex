...
\end{equation}
where ${\bf b}$ and $\bar{\bf b}$ are vectors of bosonic operators. The matrix notation here should be understand as bilinear form. For example, $\vec{b}^{\dagger} R \bar{\vec{b}}^{\dagger}$ should really mean $\sum_{ij}b^\dagger_iR_{ij}\bar{b}_j^\dagger$ where the dagger does not transpose the vector.  
...
\end{equation}
where we understand $\vec{\bar{b}}^\dagger$ as a column vector independent to $\vec{{b}}^\dagger$. Thus the whole expression can be written as
\begin{equation}
\begin{aligned}
