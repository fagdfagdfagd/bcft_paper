...
{\color{red}We first consider the process in Eq.~\eqref{eq:DDDD} for a lattice with $3000$ sites}, 
%\begin{equation}
...
Next, we analyze the more interesting $\theta$ dependent process {\color{red}described in Eq.~\eqref{eq:DNDN},}
%\begin{equation}
...
\begin{figure}
  ...
  \includegraphics[width=1\columnwidth]{PP_fit.pdf}
    ...
    \caption{The decay exponent of Loschmidt echo (a) and bipartite fidelity (b) for the process {\color{red}in Eq.~\eqref{eq:PP}}. The parameters are the same as those in Fig.~\ref{fig:DDNN}. The plot is symmetric with respect to $\theta=0.25\pi$ as predicted. The deviation around $\frac{\pi}{4}$ in (a) is small but visible, see the discussion in the main text. Inset in (a): From top to bottom, we show the power law decay of Loschmidt echo with $\theta=0.04\pi, 0.08\pi $ and $0.18\pi$. Finite size effect does not emerge before $t=10^{4}$. We use the same curve fitting method as described for Fig.~\ref{fig:DDDD}.}
    \label{fig:PPPP}
\end{figure}
...
\end{equation}
to evaluate the influence of the boundary condition $c$. It {\rm does} influence the scaling dimension, which is not captured by our analytic calculation. {\color{red}Specifically, the large value of $\theta$ still follows the quadratic relation with a deficit of $\frac{1}{8}$, while the small $\theta$ value has smaller deficit and merge with the analytic curve at $\theta = 0$. Probably change to: We note that the exponent still follows the quadratic relation with a deficit of $\frac{1}{8}$ at larger value of $\theta$. The deficit approaches zero at $\theta=0$ where the setups are the same before and after quench.}
\begin{figure}[htb]
\centering
