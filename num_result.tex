...
We use the lattice model introduced in Sec.~\ref{sec_sub:free_boson_lattice} to check the analytic results. Our numerical evaluations is based on boson Bogoliubov transformation and the explicit form of the groundstates. The readers are referred to App.~\ref{app:comp_fid_echo} for the technical details. In all the figures, we present the coefficients of the logarithmic term $\mathcal{F} / \ln L$ and $\mathcal{F} / \ln t$ and call them fidelity and echo exponents respectively. 

We start with the process
\begin{equation}
\text{DD}+\text{DD}\rightarrow\lambda+\text{DD}
\end{equation}
and show its Loschmidt echo in Fig.~\ref{fig:DDDD}. The inset is a typical Loschmidt echo diagram, whose linear decreasing behavior in the log-log axes indicates the expected power law decay. We also provide the analytic prediction $\mathcal{L}(t)\sim t^{-0.25}$ (\cf Eq.~\eqref{eq:result_DDDD}) as a dashline for comparison. The exponent (negative of the slope of the line in log-log plot) is calculated by fitting such diagrams for $\theta = 0.02n \pi$, $n = 1,...,25 $. To avoid subjective choice of data points, we repeat 5 times to randomly pick {\color{red} 10 points} before the finite size revival surges, followed by taking the mean and standard deviation. We see that the exponents all match with the $\frac{1}{4}$ theoretical line within error. 
