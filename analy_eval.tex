...
\label{fig:fidel-map}
\end{figure}
We add a small blue semi-circle to the folded strip in Fig.~\ref{fig:fidel} as the UV regulator and map it to the upper half plane using $\xi  = \exp( \frac{\pi z}{L} )$. Then both $\xi = 0$ and $1$ can host bcc operators. We assume $a = c$ such that there is only one bcc operator on the real axis enclosed by the blue semi-circle around $\xi = 1$. In order to evaluate this diagram, we add another semi-circle centered around $\xi = 1$ with radius $R_{\xi}$ (this will introduce a correction as explained in App.~\ref{app:F_correction}), and map it to the cylinder diagram on the right by $w = \ln ( \xi- 1)$. Finally the cylinder diagram can be viewed as path integral amplitude between the boundary state $b$ and $a$
\begin{equation}
...
\end{equation}
Hence the width of the cylinder is $\ln \frac{\sqrt{ \eta } - 1}{\sqrt{ \eta} + 1}$. Since conformal transformation preserves the cross ratio, the same expression can be used on the slightly non-concentric diagrams of Fig.~\ref{fig:fidel-map} and Fig.~\ref{fig:H-tau_fold}, provided we use the intersection points there to calculate the cross ratio. The calculation in Eq.~\eqref{eq:fidel_cyd_width} and Eq.~\eqref{eq:echo_cyd_width} equivalently uses the leading order approximation to $\eta$ in the respective geometry and thus get the leading order term in the width of the cylinder. The slight deviation to the precise concentric geometry will only bring $\frac{\epsilon}{L}, \frac{\epsilon}{\tau}$ corrections to the width parameter and will not affect the fidelity and echo exponents. 

Since the fidelity and Loschmidt echo are both square of the amplitudes, we should set the width of the cylinder $\beta = 2 \ln L$ or $ 4 \ln \tau$ after obtaining the partition function on it.

The actual boundary conditions on the blue lines -- the regulators in Fig.~\ref{fig:fidel-map} and Fig.~\ref{fig:H-tau_fold} are not important in the leading order. Taking Fig.~\ref{fig:H-tau_fold} for example, we can alternatively view the right panel as the amplitude between the two blue boundary states by a Hamiltonian with boundary condition $a$ and $b$
\begin{equation}
Z_{ab} =  \langle 1 | e^{ - \beta H_{ab}} | 2 \rangle  .
\end{equation}
Since $\beta$ --the width of the cylinder is large in this problem, we expect that the imaginary time evolution will project out only the groundstate $|0_{ab}\rangle $. Hence the free energy is
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where $E_c$ is the groundstate energy of $H_{ab}$. We see that different choices of the boundary conditions only change the term independent of $\beta$. Thus in the leading order we can choose any boundary condition. The one we pick is the simplest one: the periodic boundary condition that identifies the two blue lines. 

With these in mind, here we set up the partition function calculation of the general process $S_a( \theta_1 ) \rightarrow S_b( \theta_2)$ and leave the technical details to App.~\ref{app:lambda_12}. We define a set of bosonic operators related to the $a^i_n$s in Eq.~\eqref{eq:di_mode_expansion} through
\begin{equation}
\begin{aligned}
